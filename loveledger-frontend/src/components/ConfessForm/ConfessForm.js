  ...
  return (
    <div className="confess-form-container">
      ...
      <Form onFinish={handleFinish} className="confess-form">
        ...
        >
          <Input placeholder="发送方" className="input-field" />
        </Form.Item>
        <Form.Item
          name="privateKey"
          rules={[{ required: true, message: '请输入私钥!' }]}
        >
          <Input.Password placeholder="私钥" className="input-field" />
        </Form.Item>
        <Form.Item
          name="to"
          rules={[{ required: true, message: '请输入接收方地址!' }]}
        >
          <Input placeholder="接收方" className="input-field" />
        </Form.Item>
        <Form.Item
          name="title"
          rules={[{ required: true, message: '请输入表白标题!' }]}
        >
          <Input placeholder="表白标题" className="input-field" />
        </Form.Item>
        <Form.Item
          name="message"
          rules={[{ required: true, message: '请输入表白信息!' }]}
        >
          <Input.TextArea placeholder="表白信息" className="input-field" />
        </Form.Item>
        <Form.Item>
          <Button
            type="default"
        ...
        <div className="response-container">
          ...
          >
            <Space direction="vertical" style={{ width: '100%' }}>
              <div>
                <strong>Confession ID:</strong> {response.confession_id}
              </div>
              <div>
                <strong>Transaction Hash:</strong> <a href={`https://etherscan.io/tx/${response.tx_hash}`} target="_blank" rel="noopener noreferrer">{response.tx_hash}</a>
              </div>
            </Space>
          </Card>
        </div>
